...
class BacktestEngine:
    ...
    def _next_date(self, signal_date: pd.Timestamp) :
    ...
    def _get_price(self, dt: pd.Timestamp, signal: Signal) :
        """
        Get settlement price for the signal's instrument on a given date.

        For specific strikes (CE/PE): returns that option's SETTLE_PR.
        For ATM_Strangle "NIFTY 24000 CE+PE": returns sum of CE + PE settle prices.
        For EMA_Crossover index signals (NIFTY/BANKNIFTY): returns the ATM option
          settle price at the same strike as entry — NOT the average of all options
          (which would be ~index level and cause false STOP hits).
        """
        day_df = self.df[self.df["DATE"] == dt]
        if day_df.empty:
            return None

        sym_name = signal.symbol.split()[0].strip().upper()

        # ── Strangle: "NIFTY 24000 CE+PE" — return CE settle + PE settle ──────
        if "CE+PE" in signal.symbol:
            parts = signal.symbol.split()   # ["NIFTY", "24000", "CE+PE"]
            if len(parts) >= 3:
                try:
                    ...
                    return None
                sub = day_df[
                    (day_df["SYMBOL"].str.strip().str.upper() == sym_name) &
                    (day_df["STRIKE_PR"] == strike)
                ]
                ce = sub[sub["OPTION_TYP"].str.upper() == "CE"]["SETTLE_PR"]
                pe = sub[sub["OPTION_TYP"].str.upper() == "PE"]["SETTLE_PR"]
                ce_p = pd.to_numeric(ce, errors="coerce").dropna()
                pe_p = pd.to_numeric(pe, errors="coerce").dropna()
        ...
        # ── EMA / OI index signals: NIFTY or BANKNIFTY ───────────────────────
        # Use the ATM-nearest strike's average settle price (CE+PE combined / 2)
        # to stay on the same price scale as entry_price (option premium, ~1000–3000).
        # Do NOT average all strikes — that returns a meaningless blended number.
        sub = day_df[day_df["SYMBOL"].str.strip().str.upper() == sym_name]
        if sub.empty:
            return None

        # Find the strike with highest total OI (proxy for ATM)
        oi_by_strike = sub.groupby("STRIKE_PR")["OPEN_INT"].sum()
        if oi_by_strike.empty:
            return None
        atm_strike = float(oi_by_strike.idxmax())

        atm_opts = sub[sub["STRIKE_PR"] == atm_strike]
        p = pd.to_numeric(atm_opts["SETTLE_PR"], errors="coerce").dropna()
